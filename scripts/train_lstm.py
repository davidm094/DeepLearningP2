...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Entrena LSTM usando folds cacheados.")
    ...
    parser.add_argument("--dropout", type=float, default=None, help="Sobrescribe el dropout del modelo LSTM.")
...
def main() -> None:
    ...
    model_cfg = _build_model_config(cfg["models"], MODEL_NAME, data_cfg, training_cfg)
    if args.dropout is not None:
        print(f"[train_lstm] Override dropout -> {args.dropout}")
        model_cfg.dropout = args.dropout
    if args.bidirectional:
        print("[train_lstm] Activando modo bidireccional")
    ...
    for fold_idx in folds:
        ...
        print(f"[train_lstm] Fold {fold_idx} | train={x_train.shape} val={x_val.shape} | epochs={epochs}")
    ...
    print("[train_lstm] Resumen:", json.dumps(summary, indent=2))
