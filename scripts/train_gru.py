...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Entrena GRU usando folds cacheados.")
    ...
    parser.add_argument("--dropout", type=float, default=None, help="Sobrescribe el dropout del modelo GRU.")
...
def main() -> None:
    ...
    if args.dropout is not None:
        print(f"[train_gru] Override dropout -> {args.dropout}")
        model_cfg.dropout = args.dropout
    if args.bidirectional:
        print("[train_gru] Activando modo bidireccional")
    ...
    for fold_idx in folds:
        ...
        print(f"[train_gru] Fold {fold_idx} | train={x_train.shape} val={x_val.shape} | epochs={epochs}")
    ...
    print("[train_gru] Resumen:", json.dumps(summary, indent=2))
